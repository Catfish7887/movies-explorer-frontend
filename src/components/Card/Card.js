...
function Card(props) {
  ...
  const image = `${moviesApiConfig.uploadsUrl}${data.image.url}`;
  const thumbnail = `${moviesApiConfig.uploadsUrl}${data.image.formats.thumbnail.url}`;
    ...
    <>
      <li className="card">
        <div className="card__head">
          ...
          <span className="card__duration">{duration || 'Длительность фильма'}</span>
          {isSavedPage ? (
            <button onClick={dislikeCard(movieId)} className="card__favorites-button card__favorites-button_saved">
